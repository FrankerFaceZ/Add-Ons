...
export class ChatModeration extends FrankerFaceZ.utilities.module.Module {
  ...
  init() {
    ...
    document.addEventListener("contextmenu", this.boundRightClick);
    document.addEventListener("click", this.boundTimeoutClick);
  ...
  cleanup() {
    ...
    document.removeEventListener("contextmenu", this.boundRightClick);
    document.removeEventListener("click", this.boundTimeoutClick);
  ...
  onRightClick(event) {
    ...
    // Helper function to search up the React tree
    ...
    // Helper function to get the message object
    const getChatMessageObject = (element) => {
      ...
      try {
        ...
        msgObject = reactNode.pendingProps;
      } catch (_) { }
    ...
    const chatMessage = event.target.closest(selectors[settingValue]);
    ...
    if (selection.rangeCount > 0) {
      ...
      if (selectedText.length > 0 && chatMessage.contains(range.startContainer) && chatMessage.contains(range.endContainer)) {
        return;
      }
    }
    ...
    // Get message ID
    const messageObject = getChatMessageObject(chatMessage);
    const messageId = messageObject?.message?.id;
    const messageDeleted = messageObject?.message?.deleted;

    this.messageData = {
      roomId: chatMessage.dataset.roomId,
      room: chatMessage.dataset.room,
      userId: chatMessage.dataset.userId,
      user: chatMessage.dataset.user,
      messageId: messageId, // Add message ID to the data object
      messageDeleted: messageDeleted // Add message ID to the data object
    };
  ...
  createCustomTimeout() {
    ...
    selector.setAttribute("height", "224");
    selector.setAttribute("width", "80");
  ...
  handleMouseMove(e) {
    ...
    const TWO_WEEKS = 1209600; // Added ability to timeout up to two weeks
    const time = Math.min(
  ...
  removeCustomTimeout() {
    const customTimeout = document.querySelector(`#${this.CUSTOM_TIMEOUT_ID}`);
    if (customTimeout) {
      customTimeout.removeEventListener("mousemove", this.boundMouseMove);
