...
export default class ActivityFeedModeration {
  constructor(parent) {
    this.parent = parent;
  ...
  initialize() {
    const enabled = this.settings.get("addon.trubbel.channel.mod_view.activity_feed.context");
    if (enabled) {
  ...
  handleSettingChange(enabled) {
    if (enabled) {
      this.log.info("[Activity Feed] Enabling activity feed moderation");
      this.handleNavigation();
    } else {
      this.log.info("[Activity Feed] Disabling activity feed moderation");
      this.disableActivityFeedModeration();
    }
  }
  ...
  handleNavigation() {
    ...
    if (currentRoute.includes(this.router?.current?.name)) {
      ...
      if (enabled && !this.isActive) {
        this.log.info("[Activity Feed] Entering mod-view, enabling activity feed moderation");
        this.enableActivityFeedModeration();
      }
    } else {
      if (this.isActive) {
        this.log.info("[Activity Feed] Leaving mod-view, disabling activity feed moderation");
        this.disableActivityFeedModeration();
      }
    }
  }
  ...
  enableActivityFeedModeration() {
    ...
    this.log.info("[Activity Feed] Setting up right-click event listener");
  ...
  disableActivityFeedModeration() {
    ...
    this.log.info("[Activity Feed] Removing right-click event listener");
  ...
  onRightClick(event) {
    if (!this.settings.get("addon.trubbel.channel.mod_view.activity_feed.context") || !this.isActive) return;

    const element = event.target;
    const activityItem = element.closest(".activity-base-list-item");
    if (!activityItem) return;

    const react = this.site.fine.searchParentNode(activityItem, n => n?.memoizedProps?.actionsMenu);
    if (!react) {
      this.log.error("[Activity Feed] No React component found with actionsMenu");
      return;
    }
    ...
    if (userData) {
      ...
      this.log.warn("[Activity Feed] Could not extract user data from React component");
      this.log.warn("[Activity Feed] Event:", event);
      this.log.warn("[Activity Feed] UserData:", userData);
    }
  }
  ...
  getUserDataFromReact(react) {
    try {
      ...
      if (!activityProps) {
        this.log.error("[Activity Feed] No activity props found in React component");
        return null;
      }
      ...
      if (activityProps?.user === null) {
        ...
        if (activityProps?.type === "INDIVIDUAL_GIFT_SUBSCRIPTION" && anonymous) {
          ...
          if (userFragment?.token) {
            ...
            this.log.info(`[Activity Feed] Found user data in content fragments: ${userData.displayName}`);
          } else {
            this.log.warn("[Activity Feed] No user fragment found in content fragments");
          }
        } else if (activityProps?.type === "FOLLOW" && anonymous) {
          notification("⚠️", "Unknown User - no data found", 8000);
          this.log.debug("[Activity Feed] No data found:", eventData);
        } else {
          notification("⚠️", "Unknown User - no data found", 8000);
          this.log.debug("[Activity Feed] No data found:", eventData);
        }
      ...
      this.log.info(`[Activity Feed] eventData (${activityType}):`, eventData);
      this.log.debug("[Activity Feed] Extracted user data:", userData);
      return userData;
    } catch (err) {
      this.log.error("[Activity Feed] Error extracting React data:", err);
      return null;
    }
  }
      ...
      >
        ...
        {userData.timestamp && (
          <div className="mod-popup-info">
            Event time: {this.i18n.formatDateTime(new Date(userData.timestamp), "short")}
          </div>
        )}
  ...
  handleButtonClick(action, userData) {
    switch (action) {
      case "timeout-10":
        ...
        this.log.info(`[Activity Feed] Timed out ${userData.displayName} for 10 minutes`);
        break;
      case "timeout-60":
        this.parent.resolve("site.chat").ChatService.first.sendMessage(
          `/timeout ${userData.login} 3600`
        );
        this.log.info(`[Activity Feed] Timed out ${userData.displayName} for 1 hour`);
        break;
      case "ban":
        this.parent.resolve("site.chat").ChatService.first.sendMessage(
          `/ban ${userData.login}`
        );
        this.log.info(`[Activity Feed] Banned ${userData.displayName}`);
        break;
      case "profile":
        this.parent.resolve("site.chat").ChatContainer.first.onUsernameClick(userData.login);
        this.log.info("[Activity Feed] Extracted user data for profile:", userData);
        break;
      case "user-id":
  ...
  updateCSS() {
    if (this.settings.get("addon.trubbel.channel.mod_view.activity_feed.context")) {
      this.style.set("activity-feed-moderation", `
        .mod-popup {
          position: fixed;
          background: #18181b;
      ...
      this.style.delete("activity-feed-moderation");
    }
  }
}