...
export class SidebarPreview {
  ...
  getPreviewWidth() {
    const width = this.parent.settings.get("addon.trubbel.overall.sidebar-preview-size");
    return typeof width === "number" ?
      Math.max(280, Math.min(1000, width)) : 480;
  }
  ...
  updateSidebar(el) {
    if (!this.parent.settings.get("addon.trubbel.overall.sidebar-preview")) return;
    try {
      ...
      this.parent.log.error("[Sidebar Preview] Error in updateSidebar:", err);
      return false;
    }
  }
  ...
  clearSidebar(el) {
    try {
      ...
      this.parent.log.error("[Sidebar Preview] Error in clearSidebar:", err);
    }
  }
  ...
  isSidebarOnRight() {
    ...
    if (!sidebarEl) {
      this.parent.log.warn("[Sidebar Preview] Could not find sidebar element");
      return false;
    }
    ...
    this.parent.log.info(`[Sidebar Preview] Sidebar position detected: ${isOnRight ? "right" : "left"}`);
    return isOnRight;
  }
  ...
  positionPreview(card) {
    ...
    const sidebarEl = document.querySelector(".side-bar-contents, .side-nav");
    if (!sidebarEl) {
      this.parent.log.error("[Sidebar Preview] Could not find sidebar element for positioning");
      return;
    }
    ...
    this.parent.log.info(`[Sidebar Preview] Positioning preview at top: ${top}px`);
    this.previewPopup.style.top = `${top}px`;
  }

  showPreview(card) {
    if (!card) {
      this.parent.log.error("[Sidebar Preview] Missing card for preview");
      return;
    }

    this.parent.log.info("[Sidebar Preview] Showing preview for card");

    this.currentHoverCard = card;

    const react = this.parent.fine.getReactInstance(card);
    const props = react?.return?.return?.memoizedProps;

    if (!props || !props.userLogin) {
      this.parent.log.error("[Sidebar Preview] Could not find userLogin in props");
      return;
    }
    ...
    const quality = this.parent.settings.get("addon.trubbel.overall.sidebar-preview-quality");
    const muted = !this.parent.settings.get("addon.trubbel.overall.sidebar-preview-audio");
    const showTitle = this.parent.settings.get("addon.trubbel.overall.sidebar-preview-show-title");
    const showCategory = this.parent.settings.get("addon.trubbel.overall.sidebar-preview-show-category");
    const showViewers = this.parent.settings.get("addon.trubbel.overall.sidebar-preview-show-viewer_count");
    const showHypeTrain = this.parent.settings.get("addon.trubbel.overall.sidebar-preview-show-hype_train");
    const showDiscount = this.parent.settings.get("addon.trubbel.overall.sidebar-preview-show-gift_discount");
    const showGuests = this.parent.settings.get("addon.trubbel.overall.sidebar-preview-show-guests");
    const showUptime = this.parent.settings.get("addon.trubbel.overall.sidebar-preview-show-uptime");
    ...
    if (showUptime) {
      if (card._stream_meta === undefined) {
        ...
        this.parent.twitch_data.getStreamMeta(null, props.userLogin).then(data => {
          ...
          this.parent.log.error(`[Sidebar Preview] Failed to fetch uptime for ${props.userLogin}:`, err);
        });
    ...
    this.parent.log.info(`[Sidebar Preview] Setting iframe src: ${playerUrl}`);
    ...
    this.previewPopup = (
      <div
        ...
        style={{
          ...
          visibility: "visible",
          opacity: 1,
          background: `${this.parent.settings.get("addon.trubbel.overall.sidebar-preview-tooltip-background")}`,
          borderRadius: "6px",
          overflow: "hidden",
        ...
        {showTitle && streamTitle && (
          <div
            ...
            style={{
              ...
              fontSize: "1.3rem",
              fontWeight: 600,
              color: `${this.parent.settings.get("addon.trubbel.overall.sidebar-preview-tooltip-title")}`,
        ...
        {showCategory && categoryName && (
          <div
            ...
            style={{
              padding: "4px 8px",
              fontSize: "1.2rem",
              color: `${this.parent.settings.get("addon.trubbel.overall.sidebar-preview-tooltip-text")}`,
              wordBreak: "break-word",
              overflowWrap: "break-word",
              whiteSpace: "normal",
              lineHeight: 1.3,
              ...
              boxSizing: "border-box",
              overflow: "hidden",
              borderTop: `1px solid ${this.parent.settings.get("addon.trubbel.overall.sidebar-preview-tooltip-border")}`
            }}
          >
            {categoryName}
          </div>
  ...
  hidePreview() {
    if (this.previewPopup) {
      this.parent.log.info("[Sidebar Preview] Hiding preview");
