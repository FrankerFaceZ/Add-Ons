...
class VodTimestamp extends Addon {
  ...
  buildPopup() {
    ...
    container.appendChild(
      createElement('div', {
        ...
        textContent: `Enter a ${tzLabel} time from chat to get the VOD timestamp.`,
      })
    )
  ...
  async handleConvert(value, resultBox) {
    ...
    if (!parsed) {
      resultBox.textContent = 'Enter time as HH:MM:SS or HH:MM'
    ...
    if (!videoStart) {
      resultBox.textContent = 'Could not fetch VOD start time.'
      resultBox.style.color = '#eb5454'
      return
    }
    ...
    // build a Date for the entered time on the stream start date
    const entered = new Date(videoStart)
    if (useLocal) {
      entered.setHours(h, m, s, 0)
    } else {
      entered.setUTCHours(h, m, s, 0)
    }
    ...
    // show stream start for sanity checking
    ...
    resultBox.appendChild(
      createElement('div', {
        ...
        textContent: `Stream started at ${startDisplay}`,
      })
    )
    ...
    // seek the player directly for instant feedback, update URL for sharing
